...
def skip_lines(input_file, skip):
    for i in range(0, skip):
        next(input_file)    ###need a try| skip is a number
...
def clean_dimension(d, field, v):
    if is_number(v):
        if field == "weight":
            d[field] = float(v)/1000.0    #d[field]
        else:
...
def ensure_not_array(v):
    ...
    return v
def ensure_array(v):
    (is_array, v) = parse_array2(v) #could fuzhi like this
    if is_array:
        return v
    return [v]    #what's diff- v,[v]???
...
def ensure_year_array(val):
    ...
    return year_vals
##################################################
def empty_val(val):    #if have value,then value must have trailing or leading blanks
    val = val.strip()
