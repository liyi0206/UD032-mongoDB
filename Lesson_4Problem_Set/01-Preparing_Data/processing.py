...
DATAFILE = 'C:/Benben/UD032 mongoDB/Lesson_4_Problem_Set/03-Updating_Schema/arachnid.csv'
...
def process_file(filename, fields):
    ...
    for i in range(0,len(data)):  
        item1=data[i]['label']
        if "(" in item1:
            data[i]['label'],_=item1.split("(")
        item2=data[i]['name']
        if item2==None or not re.match('^[\w-]+$',item2):
            data[i]['name']=data[i]['label']
        item3=data[i]['synonym'] 
        if item3 is not None:
            data[i]['synonym']=parse_array(item3)

    for i in range(0,len(data)):
        data[i]["classification"]={}
        for key in FIELDS:
            if FIELDS[key] in ("kingdom","family","order","phylum","genus","class"):
                data[i]["classification"][FIELDS[key]]=data[i][FIELDS[key]]
                del data[i][FIELDS[key]]       
    value=data[0]['label']
    data[0]['label']=value.strip() 
    return data
